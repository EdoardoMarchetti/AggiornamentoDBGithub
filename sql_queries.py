...
def delete_table(engine, table_name, where_condition=None, chunk_size=1000, disable_pb=False):
    """
    Deletes records from the specified table in the database based on the provided WHERE condition.

    Args:
    - engine: Database engine.
    - table_name (str): Name of the table in the database.
    - where_condition (str, optional): WHERE condition for filtering rows to delete. Deletes all rows if None.
    - chunk_size (int): Size of each chunk for batch deletions. Default is 1000.
    - disable_pb (bool): Disable progress bar if True.
    ...
    try:
        ...
        # If there's a WHERE condition, add it to the query
        if where_condition:
            base_query += f" WHERE {where_condition}"

        # Start a transaction
        with engine.connect() as connection:
                connection.execute(text(base_query))
                connection.commit()  # Commit the transaction

        
        print(f"Records deleted successfully from {table_name} where {where_condition}")
    except Exception as e:
